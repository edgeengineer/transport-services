...
struct ConnectionTests {
    ...
    func testConnectionStateTransitions() async throws {
        ...
        let connection = try await withTimeout(in: .seconds(5), clock: ContinuousClock()) {
            [preconnection] in
        ...
        #expect(
            initialState == .establishing || initialState == .established || initialState == .closed
        )
    ...
    @Test(
        "Send and receive data on established connection",
    ...
    @Test(
        "Connection inherits properties from preconnection",
        .disabled("Hanging on Linux - needs investigation"))
    ...
    func testConnectionClone() async throws {
        ...
        let connection1 = try await withTimeout(in: .seconds(5), clock: ContinuousClock()) {
            [preconnection] in
            try await preconnection.initiate { event in
    ...
    func testConnectionGroup() async throws {
        ...
        let connection2 = try await withTimeout(in: .seconds(5), clock: ContinuousClock()) {
            [preconnection] in
            try await preconnection.initiate()
        }
    ...
    func testEndpointManagement() async throws {
        ...
        let newEndpoints: [RemoteEndpoint] = [
            {
                var ep = RemoteEndpoint()
                ep.ipAddress = "8.8.8.8"
                ep.port = 443
                return ep
            }(),
            {
                var ep = RemoteEndpoint()
                ep.ipAddress = "8.8.4.4"
                ep.port = 443
                return ep
            }(),
        ]
