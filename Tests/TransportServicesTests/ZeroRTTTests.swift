...
@Suite("0-RTT Tests", .timeLimit(.minutes(1)))
struct ZeroRTTTests {
    ...
    func initiateWithSend() async throws {
        ...
        // Accept connection task with timeout - start this first
        let serverTask = Task {
        ...
        let clientConnection = try await TestUtils.withTimeout(seconds: 5) {
            try await clientPreconnection.initiateWithSend(earlyData)
        }
        
        let serverConnection = try await TestUtils.withTimeout(seconds: 5) {
    ...
    func zeroRTTRejection() async throws {
        ...
        // Accept connection task with timeout
        let serverTask = Task {
            for try await connection in listener.newConnections {
                return connection
            }
